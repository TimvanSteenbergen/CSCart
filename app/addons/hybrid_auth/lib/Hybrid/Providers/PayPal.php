...
class Hybrid_Providers_Paypal extends Hybrid_Provider_Model_OAuth2
{
	...
	*/
	function initialize() 
	{
        ...
        if ($this->config["paypal_seamless"] == 'Y') {
            $this->scope = $this->scope . " https://uri.paypal.com/services/expresscheckout";
        }
        ...
        if ($this->config["paypal_sandbox"] == 'Y') {
            $this->api->authorize_url  = "https://www.sandbox.paypal.com/webapps/auth/protocol/openidconnect/v1/authorize";
            $this->api->token_info_url      = "https://api.sandbox.paypal.com/v1/oauth2/token";
            $this->api->token_url = "https://api.sandbox.paypal.com/v1/identity/openidconnect/tokenservice";
        } else {
            $this->api->authorize_url  = "https://www.paypal.com/webapps/auth/protocol/openidconnect/v1/authorize";
            $this->api->token_info_url      = "https://api.paypal.com/v1/oauth2/token";
	...
	*/
	function getUserProfile()
	{
        ...
        $_SESSION['paypal_token'] = $this->token( "access_token" );
		$response = $this->api->api("https://api" . ($this->config["paypal_sandbox"] == 'Y' ? '.sandbox' : '') . ".paypal.com/v1/identity/openidconnect/userinfo/?schema=openid"); 
