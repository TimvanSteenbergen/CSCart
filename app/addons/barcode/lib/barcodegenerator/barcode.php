...
define("BCD_C128_BAR_4"              ,   4);
  ...
  class BarcodeObject {
	...
	function DrawText ($Font, $xPos, $yPos, $Char) {
		if ($_REQUEST['type'] == 'C128C' || $_REQUEST['type'] == 'C128B' || $_REQUEST['type'] == 'C128A'){
				$xPos = $xPos-10;
		}elseif ($_REQUEST['type'] == 'I25' || $_REQUEST['type'] == 'C39'){
				$xPos = $xPos-2;			
		}
	...
	function DrawSingleBar($xPos, $yPos, $xSize, $ySize) {
	  ...
	  // If barcode image width greater than defined width - expand it
	  if ($xPos < 0) {
		  $this->mWidth += abs($xPos) + 3;
		  $xPos = 0;
	  }
