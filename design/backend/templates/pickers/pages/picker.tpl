...
<div class="clearfix">
    ...
    {if $view_mode != "list"}
        {capture name="add_buttons"}
            ...
            <div class="hidden" id="content_{$data_id}" title="{$but_text|default:__("add_pages")}">
            </div>
        {/capture}
        {if !$prepend}
...
{if $view_mode != "button"}
    ...
    {elseif !$multiple}
        <div class="input-append">
            <div class="pull-left">
                {include file="pickers/pages/js.tpl" page_id="" holder=$data_id input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button}
            </div>
            {$smarty.capture.add_buttons nofilter}
        </div>
    {/if}
    ...
    {if $multiple}
        {include file="pickers/pages/js.tpl" page_id="`$ldelim`page_id`$rdelim`" holder=$data_id input_name=$input_name clone=true hide_link=$hide_link hide_delete_button=$hide_delete_button hide_input=true position_field=$positions position="0"}
    {/if}
