...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if ($mode == 'process') {
        ...
        $pp_response['order_status'] = ($_REQUEST['STATUS'] == "5" || $_REQUEST['STATUS'] == "9") ? 'P' : 'F';
    ...
    $pp_test = $processor_data['processor_params']['mode'] == 'test' ? "https://secure.ogone.com:443/ncol/test/orderstandard.asp" : "https://secure.ogone.com:443/ncol/prod/orderstandard.asp";
    ...
    $post["ownercty"] =  $order_info['b_country'];
    $post["ownerzip"] =  trim(substr($order_info['b_zipcode'], 0, 10));
    $post["ownertelno"] =  trim(substr($order_info['phone'], 0, 30));

    $post["language"] = !empty($langs[CART_LANGUAGE]) ? $langs[CART_LANGUAGE] : "en_US";
    $post["accepturl"] = $post["declineurl"] = $post["exceptionurl"] = fn_url("payment_notification.result?payment=ogone_web&order_id=$order_id", AREA, 'current');
    $post["cancelurl"] = fn_url("payment_notification.cancel?payment=ogone_web&order_id=$order_id", AREA, 'current');
    ...
    if (isset($processor_data['processor_params']['use_new_sha_method']) && $processor_data['processor_params']['use_new_sha_method'] == 'Y') {
        ...
        $post["SHASign"] = sha1(
            "ACCEPTURL=" . $post['accepturl'] . $pp_secret .
            "AMOUNT=" . $post["amount"] . $pp_secret .
            "CANCELURL=" . $post['cancelurl'] . $pp_secret .
            "CURRENCY=" . $pp_curr . $pp_secret .
            "DECLINEURL=" . $post['declineurl'] . $pp_secret .
            (strlen($post['email'])>0? "EMAIL=" . $post['email'] . $pp_secret : "") .
            "EXCEPTIONURL=" . $post['exceptionurl'] . $pp_secret .
            "LANGUAGE=" . $post['language'] . $pp_secret .
            "ORDERID=" . $ordr . $pp_secret .
            (strlen($post['owneraddress'])>0? "OWNERADDRESS=" . $post['owneraddress'] . $pp_secret : "") .
            (strlen($post['ownercty'])>0? "OWNERCTY=" . $post['ownercty'] . $pp_secret : "") .
            (strlen($post['ownertelno'])>0? "OWNERTELNO=" . $post['ownertelno'] . $pp_secret : "") .
            (strlen($post['ownertown'])>0? "OWNERTOWN=" . $post['ownertown'] . $pp_secret : "") .
            (strlen($post['ownerzip'])>0? "OWNERZIP=" . $post['ownerzip'] . $pp_secret : "") .
            "PSPID=" . $pp_merch . $pp_secret
        );
    } else {
        //Old: SHA-1(OrderId + Amount + Currency + PSPID + additional string)
