...
class Text_Diff_Renderer_inline extends Text_Diff_Renderer {
    ...
    function _lines($lines, $prefix = ' ', $encode = true)
    {
        ...
        /*if ($this->_split_level == 'words') { // commented out by zeke
    ...
    function sideBySide($string)
    {
		...
		if ($count = preg_match_all("/(?:<del>([\s\S]+)<\/del><ins>([\s\S]+)<\/ins>)|(?:<ins>([\s\S]+)<\/ins>)|(?:<del>([\s\S]+)<\/del>)/iU", $string, $array)) {
			...
			for ($j = 0; $j < $count; $j++) {
				$string1 .= $data[$j] . ($array[1][$j] || $array[4][$j] ? '<del>' . ($array[1][$j] ? $array[1][$j] : $array[4][$j]) . '</del>' : '');
			}

			$string1 .= $data[$j];

			for ($j = 0; $j < $count; $j++) {
				$string2 .= $data[$j] . ($array[2][$j] || $array[3][$j] ? '<ins>' . ($array[2][$j] ? $array[2][$j] : $array[3][$j]) . '</ins>' : '');
			}
