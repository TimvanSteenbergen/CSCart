    ...
    {foreach from=$filters_row item="filter"}
        ...
        <div class="ty-column4">
            <div class="ty-filters-search__item">
                ...
                <div class="ty-filters-search__title">{$filter.filter|default:$filter.description|default:""}</div>
                ...
                <div class="ty-filters-search__scroll">
                    {foreach from=$filter.ranges item="range"}
                        <div class="ty-select-field"><input type="checkbox" class="ty-select-field__checkbox" name="{if $filter.feature_type == "M"}multiple_{/if}variants[]" id="variants_{$range.range_id}" value="{if $filter.feature_type == "M"}{$range.range_id}{else}[V{$range.range_id}]{/if}" {if "[V`$range.range_id`]"|in_array:$search.variants || $range.range_id|in_array:$search.multiple_variants}checked="checked"{/if} /><label class="ty-select-field__title" for="variants_{$range.range_id}">{$filter.prefix}{$range.range_name}{$filter.suffix}</label></div>
                    {/foreach}
                </div>
                {elseif $filter.feature_type == "O" || $filter.feature_type == "N" || $filter.feature_type == "D" || $filter.condition_type == "D" || $filter.condition_type == "F"}
                    ...
                    {/if}
                        ...
                        {/if}
                        <p{if !$filter.slider} class="ty-select-field"{/if}><input type="radio" name="variants[{$el_id}]" id="no_ranges_{$el_id}" value="" checked="checked" class="radio ty-select-field__checkbox" /><label class="ty-select-field__title" for="no_ranges_{$el_id}">{__("none")}</label></p>
                        {if !$filter.slider}
                            {foreach from=$filter.ranges item="range"}
                                {assign var="_type" value=$filter.field_type|default:"R"}
                                <div class="ty-select-field"><input type="radio" class="ty-select-field__checkbox" name="variants[{$el_id}]" id="ranges_{$el_id}{$range.range_id}" value="{$_type}{$range.range_id}" {if $search.variants.$el_id == "`$_type``$range.range_id`"}checked="checked"{/if} /><label class="ty-select-field__title" for="ranges_{$el_id}{$range.range_id}">{$range.range_name|fn_text_placeholders}</label></div>
                            {/foreach}
                        {/if}
                    {if !$filter.slider}
                    </div>
                    ...
                    <div class="ty-select-field">
                        ...
                        {else}
                            ...
                            &nbsp;-&nbsp;
                            <input type="text" name="{if $filter.field_type}field_range[{$filter.field_type}]{else}custom_range[{$filter.feature_id}]{/if}[to]" size="3" class="ty-input-text-short{if $search.variants[$el_id] != "O"} disabled{/if}" value="{$to_value}" id="range_{$el_id}_to" {if $search.variants[$el_id] != "O"}disabled="disabled"{/if} />
                            {if $filter.field_type == 'P'}
                ...
                {elseif $filter.feature_type == "C" || $filter.condition_type == "C"}
                    ...
                    {/if}
                    <div class="ty-select-field">
                        <input type="radio ty-select-field__checkbox" class="radio" name="ch_filters[{$el_id}]" id="ranges_{$el_id}_none" value="" {if !$search.ch_filters[$el_id]}checked="checked"{/if} />
                        <label class="ty-select-field__title" for="ranges_{$el_id}_none">{__("none")}</label>
                    </div>

                    <div class="ty-select-field">
                        <input type="radio ty-select-field__checkbox" class="radio" name="ch_filters[{$el_id}]" id="ranges_{$el_id}_yes" value="Y" {if $search.ch_filters[$el_id] == "Y"}checked="checked"{/if} />
                        <label class="ty-select-field__title" for="ranges_{$el_id}_yes">{__("yes")}</label>
                    </div>

                    <div class="ty-select-field">
                        <input type="radio ty-select-field__checkbox" class="radio" name="ch_filters[{$el_id}]" id="ranges_{$el_id}_no" value="N" {if $search.ch_filters[$el_id] == "N"}checked="checked"{/if} />
                        <label class="ty-select-field__title" for="ranges_{$el_id}_no">{__("no")}</label>
                    </div>

                    {if !$filter.condition_type}
                    <div class="ty-select-field">
                        <input type="radio" class="radio ty-select-field__checkbox" name="ch_filters[{$el_id}]" id="ranges_{$el_id}_any" value="A" {if $search.ch_filters[$el_id] == "A"}checked="checked"{/if} />
                        <label class="ty-select-field__title" for="ranges_{$el_id}_any">{__("any")}</label>
                    </div>
                    {/if}

                {elseif $filter.feature_type == "T"}
                    <div class="ty-select-field ty-nowrap">
                    {$filter.prefix}<input type="text" name="tx_features[{$filter.feature_id}]" class="ty-filters-search__input-text{if $filter.prefix || $filter.suffix}-medium{/if}" value="{$search.tx_features[$filter.feature_id]}" />{$filter.suffix}
                    </div>
                {/if}
            </div>
        </div>
...
{if $separate_form}
    ...
    <div class="buttons-container">
        ...
        &nbsp;{__("or")}&nbsp;&nbsp;<a class="cm-reset-link">{__("reset_filter")}</a>
    </div>
