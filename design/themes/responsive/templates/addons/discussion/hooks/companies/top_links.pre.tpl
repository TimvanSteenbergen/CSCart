{if $discussion && $discussion.type != "D"}
    <span class="ty-discussion__rating-wrapper" id="average_rating_{$object_type}_{$object_id}">
        ...
        {if !$discussion.disable_adding}
            <a class="ty-discussion__review-write cm-external-click cm-dialog-opener cm-dialog-auto-size" data-ca-external-click-id="discussion" data-ca-target-id="new_post_dialog_{$obj_id}">{__("write_review")}</a>
        {/if}
