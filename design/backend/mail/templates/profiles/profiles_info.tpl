...
<tr>
    <td valign="top">
        ...
        <tr>
            <td class="form-field-caption" nowrap>{__("username")}:&nbsp;</td>
        ...
        {if $send_password || $settings.General.quick_registration == "Y"}
            ...
            <tr>
                ...
                {$login_url=""}
                {if $user_data.user_type == "C" && $user_data.company_id}
                    {$login_url="?company_id=`$user_data.company_id`"}
                {/if}
                <td>{$login_url|fn_url:$user_data.user_type}</td>
            </tr>
        {/if}
