...
$pp_merch = $processor_data['processor_params']['pspid'];
$pp_secret = $processor_data['processor_params']['sha_sign'];
$pp_curr = $processor_data['processor_params']['currency'];
$pp_url = ($processor_data['processor_params']['mode'] == 'test') ? "https://secure.ogone.com:443/ncol/test/orderdirect.asp" :  "https://secure.ogone.com:443/ncol/prod/orderdirect.asp";
$pp_pass = $processor_data['processor_params']['password'];
$_order_id = $processor_data['processor_params']['order_prefix'].(($order_info['repaid']) ? ($order_id .'_'. $order_info['repaid']) : $order_id);

if (!empty($order_info['email'])) {
    $email = substr(trim($order_info['email']), 0, 50);
}
if (!empty($order_info['b_address'])) {
    $owneraddress = trim(substr(trim($order_info['b_address'] . (!empty($order_info['b_address_2'])? " ; " . $order_info['b_address_2'] : '')), 0, 35));
}
if (!empty($order_info['b_city'])) {
    $ownertown =  trim(substr(trim($order_info['b_city']), 0, 25));
}
if (!empty($order_info['b_country'])) {
    $ownercty =  trim($order_info['b_country']);
}
if (!empty($order_info['b_zipcode'])) {
    $ownerzip =  trim(substr(trim($order_info['b_zipcode']), 0, 10));
}
if (!empty($order_info['phone'])) {
    $ownertelno =  trim(substr(trim($order_info['phone']), 0, 30));
}
...
$post['CVC'] = $order_info['payment_info']['cvv2'];
if (!empty($email)) {
    $post['email'] = $email;
}
...
if (!empty($ownertown)) {
    $post['ownertown'] = $ownertown;
}
if (!empty($ownercty)) {
    $post['ownercty'] = $ownercty;
}
if (!empty($ownerzip)) {
    $post['ownerzip'] = $ownerzip;
}
if (!empty($ownertelno)) {
    $post['ownertelno'] = $ownertelno;
}
$post['Withroot'] = 'Y';
...
if (isset($processor_data['processor_params']['use_new_sha_method']) && $processor_data['processor_params']['use_new_sha_method'] == 'Y') {
    ...
    $post['SHASign'] = sha1(
        "AMOUNT=" . (100 * $order_info["total"]) . $pp_secret.
        "CARDNO=" . trim($order_info['payment_info']['card_number']) . $pp_secret.
        "CN=" . trim($order_info['payment_info']['cardholder_name']) . $pp_secret.
        "CURRENCY=" .$pp_curr . $pp_secret.
        "CVC=" . $order_info['payment_info']['cvv2'] . $pp_secret.
        "ED=" . $order_info['payment_info']['expiry_month'] . '/' . $order_info['payment_info']['expiry_year'] . $pp_secret.
        (!empty($email)? "EMAIL=" . $email . $pp_secret : '').
        "ORDERID=" . $_order_id . $pp_secret.
        (!empty($owneraddress)? "OWNERADDRESS=" . $owneraddress . $pp_secret : '').
        (!empty($ownercty)? "OWNERCTY=" . $ownercty . $pp_secret : '').
        (!empty($ownertelno)? "OWNERTELNO=" . $ownertelno . $pp_secret : '').
        (!empty($ownertown)? "OWNERTOWN=" . $ownertown . $pp_secret : '').
        (!empty($ownerzip)? "OWNERZIP=" . $ownerzip . $pp_secret : '').
        "PSPID=" . $pp_merch . $pp_secret.
        "PSWD=" . $pp_pass . $pp_secret.
        "REMOTE_ADDR=" . $_SERVER['REMOTE_ADDR'] . $pp_secret.
        $userid_condition.
        "WITHROOT=Y" . $pp_secret
    );
...
if ($response_code == '5' || $response_code == '9') {
    ...
    if (preg_match("/NCERRORPLUS=\"(.+)\"/U", $result, $m)) {
        $pp_response['reason_text'] .= "; " . $m[1];
    }

    $pp_response["order_status"] = 'F';
}

if (preg_match("/NCSTATUS=\"(.+)\"/U", $result, $m)) {
    $pp_response['reason_text'] .= " (NCStatus: " . $m[1];

    if (preg_match("/NCERROR=\"(.+)\"/U", $result, $m)) {
        $pp_response['reason_text'] .= '; ' . ((empty($error[$m[1]]) ? ("Error Code: " . $m[1]) : $error[$m[1]]));
    }
