...
{elseif $message.type == "O"}
    <div class="cm-notification-content notification-content alert-error" data-ca-notification-key="{$key}">
        <button type="button" class="close cm-notification-close" {if $message.message_state != "S"}data-dismiss="alert"{/if}>×</button>
        {$message.message nofilter}
    </div>
{else}
    <div class="cm-notification-content notification-content{if $message.message_state == "I"} cm-auto-hide{/if} {if $message.type == "N"}alert-success{elseif $message.type == "W"}alert-warning{else}alert-error{/if}" data-ca-notification-key="{$key}">
